...
export SERVICE_IP=$(kubectl get svc --namespace default $helmname-jenkins \
  --template "{{ range (index .status.loadBalancer.ingress 0) }}{{ . }}{{ end }}"); echo http://$SERVICE_IP/login
  
export SERVICE_SECRET=$(kubectl exec --namespace default -it svc/$helmname-jenkins -c jenkins -- /bin/cat /run/secrets/chart-admin-password)
